...
samples_path = 'try_ACGANwithA/ACGANwithA_Samples_1'
...
# 如果半道训练，用如下初始化方法
net_G.load_state_dict(torch.load('try_ACGANwithA/ACGANwithA_Samples_1/15_net_G.pth'))
net_D.load_state_dict(torch.load('try_ACGANwithA/ACGANwithA_Samples_1/15_net_D.pth'))
net_A.load_state_dict(torch.load('try_ACGANwithA/ACGANwithA_Samples_1/15_net_A.pth'))
...
def gradient_penalty(x, y, f):
    ...
    return gp
# 画图
def save_diagram(list_1, list_2, list_3, name,
...
# 训练函数
def train(net_G, net_D, net_A, lambda_A, c_A, sample_noise, sample_labels, output_gen_path, output_disc_path, output_A_path, output_loss_path, max_epochs=20):  # 不应该吧模型初始化放在train里面，外面初始，里面保持false
    ...
    for epoch in range(0, max_epochs):
        ...
        for i, (real_img, real_c) in enumerate(dataloader, str_i):
            ...
            with torch.no_grad():
                ...
                if (iteration % 500 == 0) or (i == len(dataloader) - 1):
                    # with torch.no_grad():
        ...
        save_diagram(list_loss_D, list_loss_G, list_loss_A, name=output_loss_path)
        torch.save(net_G.state_dict(), os.path.join(samples_path, output_gen_path))
        torch.save(net_D.state_dict(), os.path.join(samples_path, output_disc_path))
        torch.save(net_A.state_dict(), os.path.join(samples_path, output_A_path))
    return adj_A1
...
for step_k in range(16, 20):  # 为了找lambda和c而进行迭代  如果半道训练要改step_k的值
    while c_A < 1e+20:  # 当c_a小于1e+20时不断增大c_A
        adj_A1 = train(net_G, net_D, net_A, lambda_A, c_A, sample_noise, sample_labels, '%s_net_G.pth'%step_k, '%s_net_D.pth'%step_k, '%s_net_A.pth'%step_k, '%s_loss.jpg'%step_k)
