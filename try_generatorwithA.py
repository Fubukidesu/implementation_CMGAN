...
lgngenerator = LGNGenerator(adj_A = adj_A)
    ...
    for epoch_index in range(start_epoch, num_epochs):
        ...
        while more_batches:
            ...
            for _ in range(num_disc_steps):
                ...
                fake_features, adj_A1 = generator(noise, training=True)
            ...
            for _ in range(num_gen_steps):
                ...
                h_A = _h_A(adj_A1, 10).type(torch.cuda.FloatTensor)
                # print(h_A)
...
for step_k in range(20):  # 为了找lambda和c而进行迭代
    while c_A < 1e+20:  # 当c_a小于1e+20时不断增大c_A
        adj_A1 = train(lgngenerator, lgndiscriminator, train_data, val_data, 'output/models/%s_generator1.pth'%step_k, 'output/models/%s_discriminator1.pth'%step_k, 'output/models/%s_loss1.csv'%step_k, lambda_A, c_A)
        print("Optimization Finished!")

        # update parameters
        A_new = adj_A1.data.clone()  # 此时的增广后的A_new
        h_A_new = _h_A(A_new, 10)  # A_new的h(A)
