...
from lgn_module_Azaixia import LGNGenerator_Azaixia_18, LGNDiscriminator_Azaixia_18
from utils.get_CHILD18 import CHILD18
...
dataset = CHILD18(csv_file='data/CHILD/CHILD10k_num.csv')    
dataloader = DataLoader(dataset, batch_size=64, shuffle=True, num_workers=0)

# 加矩阵
adj_A = np.zeros((18, 18))
lgngenerator = LGNGenerator_Azaixia_18(adj_A = adj_A)  # 改输出个数
load_or_initialize(lgngenerator, None)

lgndiscriminator = LGNDiscriminator_Azaixia_18()  # 这里面要对应改D的输入维度
    ...
    for epoch_index in range(start_epoch, num_epochs):
        ...
        while more_batches:
            ...
            for _ in range(num_disc_steps):
                ...
                noise = Variable(torch.FloatTensor(len(batch), noise_size).uniform_())
            ...
            for _ in range(num_gen_steps):
                ...
                h_A = _h_A(adj_A1, 18).type(torch.cuda.FloatTensor)  # 修改对应特征维度
                # print(h_A)
...
for step_k in range(20):  # 为了找lambda和c而进行迭代
    while c_A < 1e+20:  # 当c_a小于1e+20时不断增大c_A
        ...
        adj_A1 = train(lgngenerator, lgndiscriminator, train_data, val_data, 'output/exp_models/lgn_model_CHILD18/%s_generator.pth'%step_k, 'output/exp_models/lgn_model_CHILD18/%s_discriminator.pth'%step_k, 'output/exp_models/lgn_model_CHILD18/%s_loss.csv'%step_k, lambda_A, c_A)
        print("Optimization Finished!")

        # update parameters
        A_new = adj_A1.data.clone()  # 此时的增广后的A_new
        h_A_new = _h_A(A_new, 18)  # A_new的h(A) 要修改对应维度
