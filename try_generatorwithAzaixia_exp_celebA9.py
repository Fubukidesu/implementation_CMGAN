...
# Bald Eyeglasses Male Mustache No_Beard Young Wearing_Lipstick Heavy_Makeup Bangs
import torch
import numpy as np
import argparse
...
from lgn_module_Azaixia import LGNGenerator_Azaixia, LGNDiscriminator_Azaixia
...
#celebA数据
data_path = 'try_LGN/data/celebA/img_align_celeba'
attr_path = 'try_LGN/data/celebA/list_attr_celeba.txt'
# attrs_default = [
...
lgndiscriminator = LGNDiscriminator_Azaixia()  # 这里面要对应改D的输入维度
    ...
    for epoch_index in range(start_epoch, num_epochs):
        ...
        while more_batches:
            ...
            for _ in range(num_gen_steps):
                ...
                h_A = _h_A(adj_A1, 9).type(torch.cuda.FloatTensor)  # 修改对应特征维度
                # print(h_A)
...
for step_k in range(20):  # 为了找lambda和c而进行迭代
    while c_A < 1e+20:  # 当c_a小于1e+20时不断增大c_A
        ...
        adj_A1 = train(lgngenerator, lgndiscriminator, train_data, val_data, 'try_LGN/output/exp_models/lgn_model_celebA9/%s_generator.pth'%step_k, 'try_LGN/output/exp_models/lgn_model_celebA9/%s_discriminator.pth'%step_k, 'try_LGN/output/exp_models/lgn_model_celebA9/%s_loss.csv'%step_k, lambda_A, c_A)
        print("Optimization Finished!")

        # update parameters
        A_new = adj_A1.data.clone()  # 此时的增广后的A_new
        h_A_new = _h_A(A_new, 9)  # A_new的h(A) 要修改对应维度
